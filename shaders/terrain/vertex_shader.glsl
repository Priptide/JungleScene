...
out vec3 position_view_space;    // the normal of the vertex in view coordinates
...
void main() {
    ...
    // vertex shader.
    vec4 pos =  PVM * vec4(position, 1.0f);

    float normalizedHeight = texture2D(map, texCoord).r;
    gl_TexCoord[0] = gl_MultiTexCoord0;
    pos.y += normalizedHeight * scale;
